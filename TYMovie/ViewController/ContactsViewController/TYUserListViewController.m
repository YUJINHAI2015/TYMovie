...
@end
@interface TYUserListViewController ()<UISearchBarDelegate, UIActionSheetDelegate, EaseUserCellDelegate, EMSearchControllerDelegate>
{
...
@implementation TYUserListViewController
...
- (void)setRightBarItem{
    ...
    [rightBtn addTarget:self action:@selector(addFriendAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setupSearchController
{
    ...
    __weak TYUserListViewController *weakSelf = self;
    ...
    [self.resultController setDidSelectRowAtIndexPathCompletion:^(UITableView *tableView, NSIndexPath *indexPath) {
        ...
        TYChatViewController *chatVC = [[TYChatViewController alloc] initWithConversationChatter:buddy
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 0) {
        [self.navigationController pushViewController:[TYApplyViewController shareController] animated:YES];
    }
    else{
        EaseUserModel *model = [[self.dataArray objectAtIndex:(section - 1)] objectAtIndex:row];
        UIViewController *chatController = nil;
        chatController = [[TYChatViewController alloc] initWithConversationChatter:model.buddy conversationType:EMConversationTypeChat];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:NSLocalizedString(@"delete Contact", @"delete Contact") delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", @"Cancel") otherButtonTitles:NSLocalizedString(@"ok", @"OK"), nil];
        [alertView show];
    }
}
...
- (void)cellLongPressAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", @"Cancel") destructiveButtonTitle:NSLocalizedString(@"friend.block", @"join the blacklist") otherButtonTitles:NSLocalizedString(@"friend.remarks", @"friend.remarks"), nil];
...
{
    NSInteger count = [[[TYApplyViewController shareController] dataSource] count];
