...
@property (nonatomic, strong) NSString *objectId; // 唯一标识
//@property (nonatomic, strong) PFACL *defaultACL; // 读写权限
...
//            if (completion) {
//                if (succeeded) {
//                    [self savePFUserInDisk:weakObj]; // 更新本地数据
//                }
...
//        }];
        ...
        // 从后台请求一个数据
        [self queryPFObjectWithCompletion:^(NSDictionary *object, NSError *error) {
//            if (object) {
//                NSData *imageData = UIImageJPEGRepresentation(img, 0.5);
//                PFFile *imageFile = [PFFile fileWithName:@"image.png" data:imageData];
//                object[kPARSE_HXUSER_AVATAR] = imageFile;
...
- (void)savePFUserInDisk:(NSDictionary*)object
{
    if (object) {
...
- (void)savePFUserInMemory:(NSDictionary*)object
{
    if (object) {
...
- (void)queryPFObjectWithCompletion:(void (^)(NSDictionary *object, NSError *error))completion
{
//    PFQuery *query = [PFQuery queryWithClassName:kPARSE_HXUSER];
